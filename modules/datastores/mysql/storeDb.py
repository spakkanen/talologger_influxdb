...
#
#                  ** 03.03.2009 v0.9e (Olli Lammi) **
#                  Class hierarchy changes. 
#
#                  ** 06.04.2009 v1.0a (Olli Lammi) **
#                  Changed to support configurable init. 
#
#                  ** 20.11.2010 v1.0b (Olli Lammi) **
#                  Added commit to insert fucntion. 
#
#                  ** 10.02.2011 v1.0c (Olli Lammi) **
#                  Interface changes. 
#
#                  ** 26.10.2011 v1.0d (Olli Lammi) **
#                  Added store filter init. 
#
#                  ** 15.01.2013 v1.0e (Olli Lammi) **
#                  Added parameter for database port.
#                  Added support for native db schema v1. 
#
#                  ** 20.02.2015 v1.0f (Olli Lammi) **
#                  With non-native schema multiple loggers can
#                  log to same table. In this case the timestamp
#                  column should be unique. Added force to UTC
#                  timezone when inserting data. DST will not
#                  work with MySQL timestamps without this.
#
#                  ** 24.02.2015 v1.0g (Olli Lammi) **
#                  Fixed broken timestamp with datetime columns.
#
#                  ** 18.01.2016 v1.0h (Olli Lammi) **
#                  Support for alternate MySQL library, MySQL
#                  Connector/Python (enabled by configuration directive).
#
#                  ** 03.01.2017 v1.0i (Olli Lammi) **
#                  Fix to check parameters if table schema 0.
#
###########################################################################
...
class DBStore(store.Store):
    ...
    def initConfiguration(self):
        ...
        try:
            ...
            try:
                if string.upper(res[0][0]) == 'TIMESTAMP':
            ...
            print "Exception SQL Query Error: " + str(e)
    ...
    def insertDataImpl(self, timeval, values):
        if self.DB_SCHEMA == 1:
            ...
            status = 1
            for val in values:
                if not self.POSITIONS.has_key(val[0]):
                    status = 0
                    break
            if status == 0:
                self.initPositions()
        ...
        try:
            ...
            if self.DB_SCHEMA == 1:
                ...
                sqlstmt = sqlstmt + string.join(valuestrs, ", ")
